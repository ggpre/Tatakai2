...
import { ArrowLeft, Filter, Star, Calendar, Play } from 'lucide-react';
...
const TVSeriesPage: React.FC = () => {
  ...
  const [selectedFilter, setSelectedFilter] = useState<'all' | 'latest' | 'top' | 'ongoing'>('all');
  const [scrollPosition, setScrollPosition] = useState(0);

  // Register for navigation
  useEffect(() => {
    if (containerRef.current) {
      registerElement('tv-series-page', containerRef.current);
      return () => unregisterElement('tv-series-page');
    }
    ...
    const handleKeyDown = (e: KeyboardEvent) => {
      switch (e.key) {
        case 'ArrowUp':
          ...
          if (showFilters) {
            // Navigate filters
            const filters: ('all' | 'latest' | 'top' | 'ongoing')[] = ['all', 'latest', 'top', 'ongoing'];
        ...
        case 'ArrowLeft':
          e.preventDefault();
          if (!showFilters && series.length > 0) {
            setSelectedIndex(prev => prev > 0 ? prev - 1 : series.length - 1);
        ...
        case 'ArrowRight':
          e.preventDefault();
          if (!showFilters && series.length > 0) {
            setSelectedIndex(prev => prev < series.length - 1 ? prev + 1 : 0);
        ...
        case 'Enter':
          ...
          if (showFilters) {
            ...
            router.push(`/anime/${selectedSeries.id}`);
          }
          break;
  ...
  }, [series, selectedIndex, showFilters, selectedFilter, currentPage, totalPages, router]);
    ...
    try {
      ...
      switch (selectedFilter) {
        ...
        case 'ongoing':
          ...
          break;
        default:
          response = await AnimeAPI.getAnimeByCategory('tv', currentPage);
          break;
      }
      ...
      if (response.success) {
        ...
        const seriesData = selectedFilter === 'all' ? animeData :
          animeData.filter((anime: Anime) => 
            anime.type?.toLowerCase().includes('tv') || 
            anime.type?.toLowerCase().includes('series') ||
      ...
      try {
        ...
        if (homeResponse.success) {
          const seriesData = homeResponse.data.latestEpisodeAnimes.filter((anime: Anime) => 
            !anime.type?.toLowerCase().includes('movie')
          );
          setSeries(seriesData);
          setTotalPages(1);
  ...
  const getFilterLabel = (filter: string) => {
    switch (filter) {
      case 'latest': return 'Recently Updated';
      case 'top': return 'Top Rated';
      case 'ongoing': return 'Currently Airing';
      default: return 'All TV Series';
    }
  };

  return (
    <div ref={containerRef} className="tv-series-page">
      <TVNavbar />
      
      <div className="tv-series-container">
        {/* Header */}
        <div className="tv-series-header">
        ...
        {showFilters && (
          <div className="tv-filter-panel">
            ...
            <div className="tv-filter-options">
              {(['all', 'latest', 'top', 'ongoing'] as const).map((filter) => (
                <button
                  key={filter}
        ...
        {!isLoading && series.length > 0 && (
          <div 
            className="tv-series-grid"
            style={{ transform: `translateY(-${scrollPosition}px)` }}
          >
            {series.map((anime, index) => (
              <div
                key={anime.id}
                className={`tv-series-card ${selectedIndex === index ? 'tv-selected' : ''}`}
              >
                <div className="tv-series-poster">
                  <img src={anime.poster} alt={anime.name} />
                  <div className="tv-series-overlay">
                    <div className="tv-series-type">
                      {anime.type || 'TV Series'}
                    </div>
                    {anime.episodes && (
                      <div className="tv-series-episodes">
                        <Play size={12} />
                        <span>
                          {anime.episodes.sub || anime.episodes.dub || 'N/A'} Episodes
                        </span>
                      </div>
                    )}
                  </div>
                ...
                <div className="tv-series-info">
                  <h3>{anime.name}</h3>
                  {anime.jname && (
                    <p className="tv-series-jname">{anime.jname}</p>
                  )}
                  <div className="tv-series-meta">
                    {anime.duration && (
                      <span className="tv-series-duration">{anime.duration}</span>
                    )}
                    {anime.rating && (
                      <span className="tv-series-rating">
                        <Star size={12} />
                        {anime.rating}
                      </span>
                    )}
